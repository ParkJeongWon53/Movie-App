...
## E2E 테스트
[cypress](https://www.cypress.io/) 설치<br>
eslint-plugin-cypress : cypress 사용시 문법에러 발생 방지.
```
npm i -D cypress eslint-plugin-cypress
```
```json
// package.json 의  "scripts" 안에 작성!!
"test:e2e": "cypress open"
```
```
npm run test:e2e
```
cypress 폴더와 cypress.json 파일이 생성된다.<br>
cypress/integration/2-advanced-examples 폴더안에 에러가 나온다면.<br>
아래 코드 설정해준다.
```js
// .eslintrc.js
module.exports = {
...
* 영화제목 검색
1. 검색(메인) 페이지로 접근한 후,
2. 영화 제목을 'frozen'으로, 펴시 개수를 30개로 입력하고,
3. 'Apply' 버튼을 클릭해 영화 목록을 검색한다.
4. 영화 목록이 30개가 잘 출력된다.
5. 영화 목록에서 'Frozen II'(겨울왕국 2) 영화 아이템을 클릭하면,
6. 영화 상세 정보 페이지로 이동한다.
7. 상세 정보 페이지에서 정보를 확인할 수 있다.
...
3. About 페이지에서 정보를 확인할 수 있다.
4. 그리고 잠시 Movie 페이지로 이동한 후,
5. Header 사용자 로고를 클리하면,
6. 다시 About 페이지에서 정보를 확인할 수 있다.
```js
...
```
* cypress 를 활용해서 화면에 보여지는 과정들이 문제가 될 수 있는 환경이 있을 수 있다.
* 터미널 활경에서 사용되는 테스터를 사용해보자.
```json
  "scripts": {
    // "dev:webpack": "webpack-dev-server --mode development",
    // "dev": "netlify dev",
    ...
    // "test:unit": "jest --watchAll",
    // "test:e2e": "cypress open",
    "test:e2e:headless": "cypress run" // 추가
  },
```
videos 폴더가 생성되며 테스트 진행과정에 녹화되어 있다.
-----
